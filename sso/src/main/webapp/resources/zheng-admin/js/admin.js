...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('winter-systemid', systemid);
		$.cookie('winter-systemname', systemname);
		$.cookie('winter-systemtitle', systemtitle);
	});
	// 显示cookie菜单

	var systemid = $.cookie('winter-systemid') || 1;
	var systemname = $.cookie('winter-systemname') || 'winter-service';
	var systemtitle = $.cookie('winter-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
